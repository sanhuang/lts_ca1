...
1. 請設計一套CI/CD Pipeline，由船廠的研發團隊部署到遠端的艦隊並符合下列要求，用文字+圖表答即可。
  - Quality
    - Security
    - Safety
    - AI導入
2. 請設計雲端地端的系統設計/網路架構，符合下列要求，主要服務有Nginx, AP, DB及其它應下列要求所需的軟硬體。
  - 網路分區與存取控制
    - HA設計
    - 資安的defense in depth
...
| `lts-api.personalwork.tw` | FastAPI Bridge：REST、Swagger `/docs`、WebSocket `/ws` |
...
```bash
make up                    # publisher ×1 + Backend + Frontend
make up PUBLISHERS=3       # 擴增 3 個 ROS GNSS publisher（自動 drone-1…3）
make up-d PUBLISHERS=2     # 背景啟動
make logs / make down
```

地圖左側可設「預期節點數」，並以點選地圖標定各 node 初始座標（與 `PUBLISHERS` 對齊即可分色看軌跡）。
...
| 服務          | URL                                                      |
| ----------- | -------------------------------------------------------- |
| 地圖          | [http://localhost:8080](http://localhost:8080)           |
| API／Swagger | [http://localhost:8000/docs](http://localhost:8000/docs) |
| WebSocket   | ws://localhost:8000/ws                                   |


三服務共用 `ltsnet`、`ROS_DOMAIN_ID=0`：Publisher（可 scale）→ `/gps/fix` → Backend → 前端 WS。
